...
<section class="content" style="padding-top: 50px; padding-bottom: 50px;">
                                ...
                                <table id="example2"
                                    ...
                                    <tr role="row">
                                        <th class="sorting_asc" tabindex="0" aria-controls="example2"
                                            ...
                                            aria-label="Rendering engine: activate to sort column descending">쿠폰 ID
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1"
                                            aria-label="Browser: activate to sort column ascending">쿠폰명
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1"
                                            aria-label="Platform(s): activate to sort column ascending">할인율
                                        </th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <c:forEach var="cvo" items="${couponListAll }">
                                        <tr role="row" class="odd">
                                            <td class="sorting_1">
                                                <a href="${pageContext.request.contextPath }/Admin/couponInfo?coupon_id=${cvo.coupon_id }&pageNum=${pm.cri.pageNum}">${cvo.coupon_id }</a>
                                            </td>
    ...
    <div class="btn-group" role="group"
         style="text-align: center; padding: 0px 0px 50px 0px;" aria-label="Basic mixed styles example">
        <button type="button" class="btn btn-primary" name="memberCouponEnroll" id="memberCouponEnroll">회원 쿠폰 발급
        </button>
    </div>
</section>
...
<div class="text-center">
    <ul class="pagination">
        <!-- 이전 -->
        <c:if test="${pm.prev }">
            <li><a href="${pageContext.request.contextPath }/Admin/couponListAll?pageNum=${pm.startPage-1 }">
                &laquo;</a></li>
        </c:if>
        <!-- 페이지 번호 -->
        ...
        <c:if test="${pm.next && pm.endPage > 0 }">
            <li><a href="${pageContext.request.contextPath }/Admin/couponListAll?pageNum=${pm.endPage + 1}"> &raquo;</a>
            </li>
        </c:if>
    </ul>
</div>
